    ...
    <DashboardLayout>
      <div className="page-card">
        ...
        <ul style={{ listStyle: "none", padding: 0 }}>
          {participantes.map((p) => (
            <li key={p.id_usuario} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              <img src={p.foto_perfil || "https://cdn-icons-png.flaticon.com/512/149/149071.png"} alt={p.nome_completo} width={40} height={40} style={{ borderRadius: "50%" }} />
              <span style={{ flex: 1 }}>{p.nome_completo}</span>
              {isCreator && p.id_usuario !== user.id_usuario && (
                <button className="btn btn-danger" onClick={() => removerParticipante(p.id_usuario)}>Remover</button>
              )}
            </li>
          ))}
        </ul>
